...
$connect = connect_sql();
$condition = "where project='$project' and year='$year' and month='$month' and (state='$status' or state='".(-$status+1)."')";
$demo_teachers = array();
if($report === '全部' || $report === '教師培訓人數')
{
  $demo_teachers = select($connect, 'demo_teachers', '*', $condition);
}
$demo_result = array();
if($report === '全部' || $report === '實施結果')
{
  $demo_result = select($connect, 'demo_result', '*', $condition);
}
$demo_public = array();
if($report === '全部' || $report === '公開授課')
{
  $demo_public = select($connect, 'demo_public', '*', $condition);
}
$demo_counseling = array();
if($report === '全部' || $report === '入校輔導')
{
  $demo_counseling = select($connect, 'demo_counseling', '*', $condition);
}
$pu021 = array();
if($report === '全部' || $report === '縣市教育局處每月填報')
{
  $pu021 = select($connect, 'PU021', '*', "where year='$year' and month='$month' and (state='$status' or state='".(-$status+1)."')");
}
$pu023 = array();
if($report === '全部' || $report === '效益執行說明(季報)')
{
  $pu023 = select($connect, 'PU023', '*', "where year='$year' and season='$month' and (state='$status' or state='".(-$status+1)."')");
}
...
<body>
  <div class="max-w-5xl mx-auto lg:pb-4">
    ...
    <div id="top" class="p-4 max-w-3xl mx-auto bg-base-200 lg:rounded-xl">
      ...
      <form class="form-control" name="form" method="post" action="./y08.php#top">
