...
$upload = $data[0]['upload'] ?? null;
$upload_1 = $data[0]['upload_1'] ?? null;
...
if (!$readonly) {
    ...
    if ($method === 'upload' || $state === '-1') {
        ...
        $directory = "./upload/public_1/$year-$month/$school_id";
        ...
        $upload_file = $_FILES['upload'] ?? null;
        $upload = upload_file($upload_file, "$directory/upload_$version");

        insert(
            $connect,
            'public_1',
            "'$project', '$year', '$month', '$school_type', '$state', '$times', '$sheet', '$people', '$text', '$upload', '$school_id', '$update_time', '$version'",
            'project, year, month, SchoolType, state, times, sheet, people, text, upload, unit, updatetime, version'
        );
    ...
    <div id="top" class="p-4 max-w-3xl mx-auto bg-base-200 lg:rounded-xl">
      ...
      <form class="form-control" name="form" method="post" enctype="multipart/form-data" action="./y14.php#top">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
          ...
          <div class="flex flex-col gap-2 bg-base-100 rounded-lg p-4">
            <label class="label gap-2">
              <span class="label-text flex-1">參加人數：</span>
              <span class="label-text-alt text-error">必填欄位</span>
            </label>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
          ...
          <div class="flex flex-col gap-2 bg-base-100 rounded-lg p-4">
            ...
            <?php if (!$readonly) { ?>
              <input id="upload" class="file-input file-input-bordered file-input-sm w-full" type="file" name="upload" onChange="set_text('upload')">
              <p id="upload_text" class="text-base opacity-50"></p>
              <input class="input input-sm input-bordered w-full" type="text" name="upload_1" placeholder="輸入影片連結" value="<?= $upload_1 ?>" <?php if ($readonly) echo 'disabled' ?> />
            <?php } ?>
            <?php if ($upload) {
              $file_path = explode('/', $upload) ?>
              <a class="btn btn-sm btn-outline" href="<?= $upload ?>" download><?= end($file_path) ?></a>
            <?php } ?>
          </div>
        </div>
